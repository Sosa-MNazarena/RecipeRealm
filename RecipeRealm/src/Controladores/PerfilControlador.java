...
public class PerfilControlador implements PerfilRepository {
    ...
    public Perfil autenticar(String correo, String contrasena) {
        Perfil perfil = null;
        try {
            PreparedStatement statement = connection.prepareStatement("SELECT id_usuario, nombre, pseudonimo, correo, contrasena, descripcion, verificado FROM usuarios WHERE correo = ? AND contrasena = ?");
            ...
            if (resultSet.next()) {
                ...
                perfil = new Perfil(idUsuario, nombre, pseudonimo, correoDb, contrasenaDb, descripcion, verificado);
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
